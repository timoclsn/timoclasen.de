...
const RunningStatsInner = async () => {
  const runningData = await query.sports.running();
  ...
  const distanceLabels = [
    ...(lastRun.distance.raw >= distanceThreshold
      ...
      : []),
    ...(lastRun.distance.raw >= thisYear.farthest ? [yearsBestLabel] : []),
  ];
  ...
  const heartrateLabels = [
    ...(lastRun.avgHeartrate.raw < heartrateThreshold
      ...
      : []),
    ...(lastRun.avgHeartrate.raw <= thisYear.lowest ? [yearsBestLabel] : []),
  ];
